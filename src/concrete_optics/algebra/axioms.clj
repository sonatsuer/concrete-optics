...
(defn compose-functor-axiom
  ...
  (let [fmap (.fmap applicative)]
    (= (fmap f (fmap g x)) (fmap (comp f g) x))))
...
(defn lifted-associativity-axiom
  ...
  (let [associator (fn [left-associated]
    ...
    (= ((.fmap applicative) associator (lifted-pair (lifted-pair fx fy) fz))
...
(defn lifted-left-unit-axiom
  ...
  (let [lifted-pair ((.binary-lift applicative) (fn [x y] [x y]))]
    (= ((.fmap applicative) first (lifted-pair fx ((.unit applicative) :throw-away)))
       fx)))
...
(defn lifted-right-unit-axiom
  ...
  [applicative fx]
  (let [lifted-pair ((.binary-lift applicative) (fn [x y] [x y]))]
    (= ((.fmap applicative) second (lifted-pair ((.unit applicative) :throw-away) fx))
       fx)))
